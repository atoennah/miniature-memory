...
class GPTConfig:
    ...
    Using a dataclass provides a structured, type-safe schema for the model's
    hyperparameters, aligning with the principle of Ontological Clarity.
    """
    vocab_size: int
    block_size: int
    n_layer: int
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[Tuple[torch.Tensor, torch.Tensor]] = None) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Note: T_out is the full sequence length (including cache), while q is only the new tokens.
        T_out = k.size(1)
        k = k.view(B, T_out, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T_out, hs)
        q = q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = v.view(B, T_out, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T_out, hs)

        # Causal self-attention using PyTorch's fused kernel
        # When using KV cache, we only need causal masking if we are processing more than one new token (T > 1).
        # If T=1, the new token can attend to all past tokens in the cache and itself.
        is_causal = (T > 1)
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None, kv_cache: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor], List[Tuple[torch.Tensor, torch.Tensor]]]:
        ...
        if kv_cache is not None:
            # If using KV cache, we assume idx contains ONLY the new tokens.
            # We need to calculate their positions based on the cache length.
            past_length = kv_cache[0][0].size(1)
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            x, layer_past = block(x, kv_cache=layer_past)
            new_kv_cache.append(layer_past)
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int, temperature: float = 1.0, top_p: float = 0.9) -> torch.Tensor:
        ...
        Utilizes a KV cache for O(N) generation complexity.
        """
        self.eval()
        kv_cache = None

        # Initial forward pass to fill the cache with the prompt
        # We only need the logits for the last token of the prompt.
        logits, _, kv_cache = self(idx, kv_cache=None)

        for _ in range(max_new_tokens):
            # We only pass the last token to the model when using KV cache
            ...
            # Forward pass for the next token, using and updating the cache

...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, past_key_value: Optional[Tuple[torch.Tensor, torch.Tensor]] = None) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Sliding window truncation: ensure cache does not exceed block_size
        if k.size(2) > self.config.block_size:
        ...
        # When using KV cache and T=1, causality is implicit.
        # For T > 1 (e.g., initial prompt), we need is_causal=True.
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, past_key_value: Optional[Tuple[torch.Tensor, torch.Tensor]] = None) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        attn_out, next_kv = self.attn(self.ln_1(x), past_key_value=past_key_value)
        x = x + attn_out
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, targets: Optional[torch.Tensor] = None, past_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor]]] = None) -> Tuple[torch.Tensor, Optional[torch.Tensor], List[Tuple[torch.Tensor, torch.Tensor]]]:
        """Forward pass for the GPT model with support for stateful KV-caching."""
        b, t = idx.size()

        if past_key_values is None:
            past_length = 0
        else:
            # All layers have same cache length, we take the first one
            past_length = past_key_values[0][0].size(2)

        total_length = past_length + t

        # Determine position indices, accounting for sliding window if necessary
        ...
        # Transformer blocks with KV-cache propagation
        new_past_key_values = []
        for i, block in enumerate(self.transformer.h):
            pkv = past_key_values[i] if past_key_values is not None else None
            x, next_kv = block(x, past_key_value=pkv)
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int, temperature: float = 1.0, top_p: float = 0.9) -> torch.Tensor:
        """
        Autoregressively generates a sequence of tokens using stateful KV-caching for O(N) efficiency.
        """
        self.eval()
        past_key_values = None

        # Initial forward pass to populate cache with prompt
        # If prompt is longer than block_size, truncate it first
        if idx.size(1) > self.config.block_size:
            idx_input = idx[:, -self.config.block_size:]
        else:
            idx_input = idx

        logits, _, past_key_values = self(idx_input, past_key_values=None)

        for _ in range(max_new_tokens):
            # Only process the single last token
